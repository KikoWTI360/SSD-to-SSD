...
#
# La firma e' ad-hoc: l'app gira su questo Mac e ottiene i suoi entitlements
# (quindi la sandbox e' attiva come in produzione), ma non e' distribuibile.
# Per una build firmata col tuo Team apri il progetto in Xcode.
#
# Nota: il codice eseguibile di questo script resta volutamente in ASCII puro e
# usa sempre ${VAR} con le graffe. Con un locale non UTF-8 bash inghiotte i byte
# di un carattere accentato adiacente a una variabile dentro il nome stesso.
set -euo pipefail
...
if [ ! -d "${BUILT_APP}" ]; then
    echo "Compilazione riuscita ma il bundle non e' dove me lo aspettavo:" >&2
    echo "    ${BUILT_APP}" >&2
    exit 1
fi
...
# Guardia: rimuoviamo solo un bundle che si chiama davvero cosi'.
if [ -e "${DESTINATION}" ]; then
    if [ -d "${DESTINATION}" ] && [ "$(basename "${DESTINATION}")" = "SSDCopier.app" ]; then
        rm -rf "${DESTINATION}"
    else
        echo "Mi rifiuto di sovrascrivere ${DESTINATION}" >&2
        echo "perche' non e' un bundle SSDCopier.app." >&2
        exit 1
    fi
fi
